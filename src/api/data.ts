...
const niver = converterDataParaAmericano(data.birth_date)
        const query = 'INSERT INTO credentials (Name, Email, BirthDate, Plan, Login, Phone, Password) VALUES (?, ?, ?, ?, ?, ?, ?)';
        //const nascimento = converterDataParaAmericano(data.birth_date);
        db.query(query, [data.full_name, data.email, niver , data.plan, data.username, data.phone, data.password], (err, results) => {
            if (err) {
                reject(err);
            } else {
                resolve(results);
